 ...
 */
 ...
 enum custom_layers {
     _BL,
     _FL,
     _NAV, // New navigation layer
 };
 
 // No Tap Dance needed anymore
 ...
 const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
     ...
     [_BL] = LAYOUT_65_ansi_blocker(
       ...
       MT(_NAV, KC_LCTL),  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,  KC_ENT,             KC_PGDN,  // MOD-TAP HERE
     ...
     /* Keymap _NAV: Navigation Layer (Accessed via LCtrl)
      */
 ...
 bool process_record_user(uint16_t keycode, keyrecord_t *record) {
   switch (keycode) {
     case MT(_NAV, KC_LCTL): // Handle the Mod-Tap key
       if (record->tap.count > 0 && record->event.pressed) {
         // Tap, send LCtrl
         tap_code(KC_LCTL);
         return false;  // Skip default handling
       }
       break; // Important: Let the default Mod-Tap handling occur for hold
   }
